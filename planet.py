class Planet():
    ...
    def __str__(self):
        return (f"Планета {self.name} | Радиус {self.radius} | Масса {self.mass} | Расстояние до солнца в км {self.distance} | Тип {self.pl_type} | id {self.id}")
    ...
    def to_string(self):
        return f'{self.name};{self.radius};{self.mass};{self.distance};{self.pl_type}'
